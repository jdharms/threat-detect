...
func TestSqliteAddIPDetails(t *testing.T) {
	...
	myMock.ExpectPing()
	myMock.ExpectExec("CREATE TABLE IF NOT EXISTS detail").WillReturnResult(sqlmock.NewResult(1, 1))
	myMock.ExpectExec("INSERT INTO detail").WithArgs(testDetails.UUID, testDetails.CreatedAt, testDetails.UpdatedAt, testDetails.ResponseCode, testDetails.IPAddress).WillReturnResult(sqlmock.NewResult(1, 1))
	myMock.ExpectClose()
